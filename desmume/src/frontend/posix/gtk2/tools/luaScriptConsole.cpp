...
static void console_append(LuaConsole *con, const char *str)
{
	...
	// GTK2's text buffer mishandles \r in \r\n sequences — strip \r before insert
	...
	gtk_text_buffer_insert(con->output_buf, &end, text, -1);
...
static void on_browse_clicked(GtkButton * /*btn*/, gpointer user_data)
{
	...
	// GTK2 uses GtkFileChooserDialog instead of GtkFileChooserNative
...
void lua_script_close_all()
{
	...
	// Synthesize a GDK_DELETE event — mirrors gtk_window_close() from GTK3
	for (int i = (int)windows.size() - 1; i >= 0; i--) {
		GtkWidget *w = windows[i];
		GdkWindow *gdk_win = gtk_widget_get_window(w);
		if (gdk_win) {
			GdkEvent *event = gdk_event_new(GDK_DELETE);
			event->any.window = GDK_WINDOW(g_object_ref(gdk_win));
...
void lua_script_open_console(GtkWindow *parent)
{
	...
	// GTK2: use gtk_vbox_new instead of gtk_box_new(GTK_ORIENTATION_VERTICAL)
	...
	// GTK2: use gtk_hbox_new instead of gtk_box_new(GTK_ORIENTATION_HORIZONTAL)
	GtkWidget *btn_row = gtk_hbox_new(FALSE, 4);
	...
	gtk_box_pack_start(GTK_BOX(btn_row), gtk_hbox_new(FALSE, 0), TRUE, TRUE, 0);
	...
	// ── Row 3: stdout checkbox (right-aligned) ──
	GtkWidget *stdout_row = gtk_hbox_new(FALSE, 0);
	...
	// GTK2: set monospace font via Pango (no CSS)
	PangoFontDescription *font = pango_font_description_from_string("Monospace 10");
	gtk_widget_modify_font(con->output_view, font);
	...
	// ── Drag-and-drop ──
	gtk_drag_dest_set(con->window, GTK_DEST_DEFAULT_ALL, NULL, 0, GDK_ACTION_COPY);
